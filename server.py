...
class Root:
    ...
    def is_logged_in(self):
        """ Returns true if your session username is in
    ...
    def insert(self, **args):
        ...
        cherrypy.session.acquire_lock()
        assert self.is_logged_in(), "User is not logged in"
        assert "doc_name" in args and "pos" in args and "t" in args and "ss" in args and "cs" in args, "Bad request- please specify doc_name, pos (caret position), t (text), ss (server state), and cs (client state)"
        d = self.gb.get_document_by_name(args['doc_name'])
        assert d.is_subscribed(cherrypy.session['user']), "You must be subscribed to this document to do that."
        d.insert(cherrypy.session['user'], int(args['pos']), args['t'], int(args['ss']))
    insert.exposed = True
    ...
    def remove(self, **args):
        ...
        cherrypy.session.acquire_lock()
        assert self.is_logged_in(), "User is not logged in"
        assert "doc_name" in args and "begin" in args and "end" in args and "ss" in args and "cs" in args, "Bad request- please specify doc_name, pos (caret position), t (text), ss (server state), and cs (client state)"
        d = self.gb.get_document_by_name(args['doc_name'])
        assert d.is_subscribed(cherrypy.session['user']), "You must be subscribed to this document to do that."
        d.remove(cherrypy.session['user'], int(args['begin']), int(args['end']), int(args['ss']))
    remove.exposed = True
